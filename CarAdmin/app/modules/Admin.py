...
class OrderView(AdminModel):
    ...
    column_formatters = dict(pay_at=lambda v, c, m, p: formatPayAt(m.pay_at),
                             ...
                             fromdate = lambda v, c, m, p: MasterData[0] if getMasterData(m.id) else m.fromdate,
                             todate=lambda v, c, m, p: MasterData[1] if MasterData else m.todate,
                             kmbefore=lambda v, c, m, p: MasterData[2] if MasterData else m.kmbefore,
                             kmafter=lambda v, c, m, p: MasterData[3] if MasterData else m.kmafter,
